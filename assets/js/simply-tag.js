...
var delcarationAutoSuggest = function ($) {
    $( document ).ready(function() {
          $('#test').simplyTag(
            {
                forMultiple: true,                      
                dataSource: JSON.parse($('#cat_list').val())
            });
            $('.nfc_widget').find('.simply-tags').append($('#favo_cats').val());
            $('.nfc_widget').find('.simply-tags').find(".tag [data-role='remove']").on('click', function () {
                let catid = $(this).parent().attr('data-key');
              ...
              $('#test2').simplyTag(
                {
                    forMultiple: true,                      
                    dataSource: JSON.parse($('#tag_list').val())
                });
                 $('.nfc_tag_widget').find('.simply-tags').append($('#favo_tags').val());
                 $('.nfc_tag_widget').find('.simply-tags').find(".tag [data-role='remove']").on('click', function () {
                    //let catid = $(this).parent().attr('data-key');
                
                    let tag_id = $(this).parent().attr('data-key');
...
//----------------------------------------------------------------------------------------------
  ...
  //-------------------------------------Add Item-------------------------------------
  let catid = suggestionElement.attr('data-key');
